...
(() => {
    ...
    async function qe() {
        ...
        l.replace(y, "").split(",").forEach(Te => {
            ...
            if (P && W) {
                ...
                for (let b = 0; b < n.length; b++) P.endsWith(n[b]) && (v = Number(v));
                for (let b = 0; b < a.length; b++) P.startsWith(a[b]) && (v = v === "true");
                t[P] = v
            }
        }), y = y.replaceAll("'", "").replaceAll('"', "");
        let K = y.indexOf(":["),
            ve = y.slice(K + 2, y.length - 1);
        return t.blackUrlList = ve.split(","), U.storage.local.set({
            defaultConfig: t
        }), t
    }
    var L = {
    ...
    async function Xe(e) {
        var n;
        let t = await h.API();
        return ((n = e.websiteURL) == null ? void 0 : n.indexOf("tbi.com.iq")) !== -1 && (e.websiteURL = "https://apps.tbi.com.iq/dollar/register.aspx"), await t.createTaskResult({
            task: {
